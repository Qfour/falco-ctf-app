...
<!-- Source: scenarios/nimbusbreach-with-tutorial/scenario.yaml `challenges:` + challenges/*/falco-rule.yaml `attack:` blocks. Regenerate: make gen-attack. -->

# ATT&CK カバレッジ — CTF Company — Full Scenario + Tutorial (11 missions) (`nimbusbreach-with-tutorial`)

このシナリオが含む 11 課題の Tactic / Technique 対応表 (scenario.yaml の `challenges:` 順)。全課題のグローバルカバレッジは `challenges/ATTACK-COVERAGE.md` を参照。
...
宣言: Discovery, Credential Access, Stealth, Execution, Command and Control, Exfiltration

実際: Execution, Discovery, Credential Access, Stealth, Command and Control, Exfiltration
