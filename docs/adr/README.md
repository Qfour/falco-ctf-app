...
| [0014](0014-journey-narrative-scenario-overlay.md) | journey narrative を課題ローカルからシナリオ所有のオーバーレイへ分離 (P27) | **Proposed** | `journey.yaml` の `briefing`/`bridge` を課題ローカル fallback + シナリオ側 additive override の2層にする。既存 journey.yaml は後方互換で無変更。**実装は別PR (P27-2)、landing まで Accepted にしない** | — |
