...
- **10 は auto-solve 経路 (exfil → Sweeper) と手動 submit の両方**を確認する
  ...
  > **⚠ この 1 項目は [ADR-0004](0004-capstone-dual-path-e2e-order.md) で部分 supersede された**
  > (2026-08-19)。**要求「10 で両経路」は維持**されるが、**理由が差し替わり順序が固定**された ——
  > 「共有が壊れていないこと」は既に unit test が pin しており、gate 3 は実は共有されておらず
  > **二重実装**されている。実行手順 (d′) と順序制約は ADR-0004 を見よ。
  > **(d) の他の記述・(a)(b)(c)(e)・Decision・I11 昇格条件はすべて有効。**
