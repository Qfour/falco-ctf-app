...
func TestAuthz_AllDeclaredGatesEnforced(t *testing.T) {
	...
	// difference. 7 admin + 11 self(-write) + 8 none/claimed-identity = 26,
	// matching ADR-0005/ADR-0006/app#116/app#84's real-world route-count
	// canon (apispec_parity_test.go's TestAPISpec_V1_RouteSetMatchesSpec).
	if adminGated != 7 {
	...
	if openGated != 8 {
		t.Fatalf("expected exactly 8 Authz: none/claimed-identity routes (GET /portal, GET the cybercore css asset, GET the design-tokens css asset (app#116), POST /falco/events, GET /healthz, GET /metrics, POST /api/challenges/{cid}/submit, POST /internal/exfil/{cid}), got %d", openGated)
	}
}
