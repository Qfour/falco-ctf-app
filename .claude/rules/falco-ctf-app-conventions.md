...
| Falco custom rule override (ADR-0008, **project 初の default ruleset override**) | `falco-ctf-platform/helmfile/releases/falco/values.yaml.gotmpl` の `customRules` block が正典。app 側は `challenges/custom-falco-rules.txt` (customRules として追加されたルール名の allowlist manifest。1行1ルール名) を持ち、`scripts/check-challenge-rules.sh` (必須 CI check `challenge-rules`) が「upstream `falcosecurity/rules` ∪ このマニフェスト」を既知集合として `challenges/*/falco-rule.yaml` の `expectedRules`/`forbiddenRules` を検証する。**ルール名は両リポで文字列完全一致させる手動規律** (自動整合機構は無い — `challenges/*/rule.yaml` の「デプロイ中の実ルールセットから再抽出する」既存運用と同種)。**デプロイ順序依存**: `requireExpectedRuleFire: true` を持つ evade challenge (現状 05) は、platform 側の対応する customRules が実クラスタで稼働していない限り永久に unsolvable になる (`internal/scoreboard/scoring` の `evaluateClean` が該当ゲートを無条件に評価するため、feature flag 等の緩和は無い)。**platform の customRules landing → app 側の `requireExpectedRuleFire: true` 有効化、の順を守ること** (逆順で本番投入すると mission が softlock する)。契約変更は両 repo 同時 PR + 相互リンク必須。**[ADR-0025](../../docs/adr/0025-evade-flag-placement-separation.md) (`customRules` 3 件目)**: mission03 (`03-stealth-read`)/mission10 (`10-final-exfil`) の evade flag を `/opt/nimbus/vault/{creds.recover,master.key}` へ retarget し、`Read sensitive file untrusted` へ **dir-prefix match** (`fd.name startswith "/opt/nimbus/vault/" and container.name != "plant"`) を append (`rules-nimbus-vault-sensitive-file.yaml`) — mission05 の exact-match append とは条件の形が異なる (1 エントリで 2 課題を同時カバーする設計、ADR-0025 §Context)。**デプロイ順序依存 (mission05 とは逆方向の失敗モード)**: mission05 は customRules 未 landing だと `requireExpectedRuleFire` で **softlock** (恒久 unsolvable) するのに対し、mission03/10 は逆に customRules 未 landing のまま app 側の新 plant-target (`/opt/nimbus/vault`) が先に有効化されると `cat /opt/nimbus/vault/...` が無検知になり **free-win** (evasion 技法なしで自明に solve) する。**platform → app の順を守ること**は同じだが、逆順で壊れる方向が softlock/free-win で真逆である点に注意 |
