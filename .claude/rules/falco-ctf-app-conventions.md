...
| I5 | 全 8 イメージ (scoreboard / auth-policy / collector / ttyd / ttyd-proxy / challenge / docs / detect-grader) を **同一 git SHA** でビルド・push。detect-grader は `type: detect` 課題の capture-replay 採点 (falco base + grade.sh)。ttyd-proxy は P23-3 の CSP frame-ancestors リバースプロキシ。#44 で CEO 批准済 (6→7)、7→8 は P23-3 で CEO 承認 (本 PR) |
...
| ttyd-proxy 配線 (P23-3, **正典**) | ctf-user workspace Pod 内で ttyd の前段に立つ CSP frame-ancestors リバースプロキシ。契約: (1) image = `falco-ctf/ttyd-proxy:<git-sha>` (I4/I5)、(2) ttyd-proxy が Ingress 到達ポート (`LISTEN_ADDR` env、既定 `:7681` = 現行 ttyd ポート) を listen し、ttyd を localhost のみの別ポートへ退避 (`UPSTREAM_ADDR` env、既定 `http://127.0.0.1:7682`。ttyd 側は `TTYD_PORT=7682` + `--interface 127.0.0.1` 相当に変更が必要)、(3) `FRAME_ANCESTORS` env (既定 `'none'` = fail-safe、何も frame できない) — platform が P23-4 でポータル origin (`https://ctf-event.<dnsSuffix>` 等) を注入。chart 側の container/env/Service.targetPort 配線は `charts/ctf-user` (本 repo) に app-lead が追って反映 (本 PR の範囲外)。契約変更 (ポート番号・env 名) は両 repo 同時 PR |
